...
================================================================================
七、千乘探索 星载 OS / 系统软件岗——星载 Linux 裁剪 / PREEMPT_RT / 星务 BSP / SEU / 在轨热补丁
================================================================================
（与银河航天同赛道：Zynq+嵌入式 Linux+SEU+kpatch 可复用，本节侧重星务计算机 BSP 与在轨上注，避免与银河航天重复话术）

Q1：星载 Linux 怎么裁剪？星务计算机 BSP 与地面嵌入式有什么不同？
答题要点：
- 诚实：简历未做过星载 OS，但闻翔 Zynq 上做过 Petalinux 移植 uboot/kernel/rootfs 裁剪，套路可迁移。
- 裁剪思路：内核 menuconfig 关闭不需要子系统（声卡/蓝牙/部分文件系统）；rootfs 用 Buildroot/Yocto 精简到几 MB；驱动按需编进或模块化（星载一般静态编进，避免模块加载失败）；裁掉 debug 符号减体积。
- 星务 BSP 特点：星务计算机管电源/遥测/遥控/姿控，BSP 要稳定优先于性能；外设以低速总线为主（I2C/SPI/UART 接姿控敏感器、磁强计、太阳敏感器），时序确定性比带宽重要。
- 与地面区别：星载资源受限（SRAM/Flash 小）、长寿命（5-15 年）、单粒子环境、上行带宽窄（更新要小补丁不能整机刷）。
- 可讲：我移植过 uboot/kernel/rootfs 全栈 + 设备树驱动，星务 BSP 的 bring-up 流程一样；不同在于要叠加容错（看门狗/三机冗余）和可上注更新。

Q2：PREEMPT_RT 在星载能上吗？和简历的 15 万并发 <100us 软实时有什么关系？
答题要点：
- 诚实：简历未做过 PREEMPT_RT 内核改，15 万并发 <100us 是软实时优化（隔离+减开销），不是硬实时抢占改造。
- PREEMPT_RT 原理：把内核大部分自旋锁改成 rt_mutex（可睡眠）、强制线程化中断，让几乎所有内核路径可抢占，把调度延迟压到微秒级抖动可控。
- 星载权衡：姿控/姿态确定这类硬实时环，传统做法是裸核或 RTOS（如 RTEMS/VxWorks）做控制环，Linux 做星务管理；如果全上 Linux，PREEMPT_RT + isolcpus + 线程化中断是路径，但要测长尾抖动（最坏情况比平均更重要）。
- 与简历迁移：我的"软中断绑核+中断聚合+锁拆分"思路是减抖动的工程手段，可叠加在 PREEMPT_RT 之上进一步压延迟；PREEMPT_RT 本身的内核改我坦承没做过但理解原理。
- 可追问"延迟怎么测"：cyclictest 跑调度延迟分布，看 max 而非 avg；简历里我用 perf/ftrace 测中断到用户态时间分布，方法可迁移。

Q3：单粒子翻转 SEU 在星载怎么处理？简历的 bit 翻转定位经验怎么用？
答题要点（核心复用点，与银河航天 Q3 同源但侧星务）：
- 简历经验：硬件随机单 bit 翻转 → do_page_fault 宕机，crash 看页表项被改，对比正常页表项定位；ARM RAS 调研做过内存错误注入全链路（固件 PSCI/ERRIDR → SDEI → 内核 EDAC → rasdaemon）。
- SEU 比地面 bit 翻转严重：高能粒子穿透，翻转率高 1-2 个数量级，且有 MBU（多 bit 翻转）风险，单字节 ECC 不够。
- 星载对策分层：
  - 内存：ECC 内存（SEC-DED 纠单 bit 检双 bit）；关键数据区用 TMR 三模冗余；页面隔离（出错页标记隔离不再用）。
  - FPGA 配置区：scrubbing（周期回读 CRC 并重配，防止配置 RAM 被翻转导致逻辑错）。
  - 关键变量：软件 CRC/校验和复算；关键控制流冗余执行+表决。
  - 看门狗：硬看门狗兜底，跑飞自动复位。
- 与简历迁移：我做过 EDAC/SDEI 上报链路实验，理解"硬件检测→上报→内核处理→用户态可见"全链路；星载 SEU 处理在这套链路上叠加冗余与 scrubbing。
- 可追问"SEC-DED 为什么不够"：SEC-DED 只纠单 bit；MBU 是一个粒子打翻相邻多 bit，需要更强的纠错码（如 Reed-Solomon）或字间交织（interleaving）让相邻 bit 落在不同码字。

Q4：kpatch 在轨上注怎么做？空间环境能直接用吗？
答题要点（与银河航天 Q5 同源，侧重上注流程与限制）：
- 简历：kpatch 产品化——仿红帽 rpm spec 打包 + 多版本分发，做的是产品化（打包/灰度/回滚），不是 livepatch 框架本身开发。
- 在轨上注流程：地面验证 patch → 上行（带宽窄，patch KB 级合适）→ 灰度加载到一个核 → 验证 → 全核生效 → 异常自动回滚（livepatch disable）。
- 空间适配难点：
  - 单粒子可能让热补丁本身失配——补丁加载后要校验完整性。
  - 旧函数栈长时间不退出（长任务）会卡 stop_machine 等待——星务长任务要拆分或允许跳过。
  - 看门狗兜底：上注期间看门狗必须保持，卡 stop_machine 超时强制复位。
- 诚实：kpatch 产品化我做打包分发，livepatch 一致性模型（shadow stack/grace period）是社区现成机制；在轨场景的容错叠加是设计层面，没实操过星载热补丁。

Q5：CCSDS/ECSS 星载标准了解吗？星务通信协议怎么做？
答题要点：
- 诚实：CCSDS/ECSS 没做过，坦承未做过但理解大意。
- CCSDS（空间数据系统咨询委员会）：定义空间通信协议族——TM/TC（遥测/遥控）空间包、AOS（高级在轨系统）、File Delivery Protocol（CFDP 用于在轨文件传输）。
- ECSS（欧洲空间标准化）：欧航局标准，规定空间软件工程/可靠性/验证流程，比 CCSDS 更偏工程规范。
- 与简历迁移：我做过串口协议分包设计（基带 idle 中断接收 + 分包），星务遥控/遥测帧格式设计思路一致（帧头+长度+校验+分包重组）；CCSDS 具体帧格式我没实现过，但协议栈实现能力可迁移。
- 可讲：如果要实现 CCSDS 空间包，我能基于现有协议栈经验快速上手，关键是要对帧同步、CRC、重传（CFDP）这些机制熟悉。

================================================================================
八、地平线 底层系统 / 内核岗——智驾 OS 实时性 / 车规内核长期维护 / kpatch OTA / RAS ASIL / BPU 新架构适应力
================================================================================

Q1：上交所 ARM 低延时 15 万并发 <100us 怎么做到的？怎么迁移到车规延迟敏感场景？
答题要点（第一亮点，要讲透迁移逻辑）：
- 场景：股票交易小包高并发 UDP，单容器 15 万包/秒，端到端平均 <100us。
- 手段分层：
  1. 网络栈参数：busy_read/busy_poll 让 socket 主动轮询网卡队列；减 netdev_budget；关不必要功能。
  2. 软中断绑核（关键）：RPS/RSS 绑网卡多队列到指定核；XPS 设发送队列 CPU 亲和；isolcpus 隔离业务核；NET_RX 软中断固定到业务核。
  3. 中断聚合：低延时场景反方向调（降低 rx-usecs）让中断及时上来；或 NAPI 轮询绕中断。
  4. 独立写网卡中断展示内核模块 ~300 行：把网卡接收中断单独绑核+轻量化处理，绕过完整协议栈。
  5. 锁优化：ebt_do_table 读写锁拆分 + lse 原子锁，解决软中断占满 CPU。
  6. gazelle/dpdk：dpdk 完全旁路内核，gazelle 半旁路。
- 迁移到车规：
  - 智驾感知/规控数据链路对延迟和抖动敏感，方法论一致：隔离（isolcpus/绑核）+ 减开销（中断聚合/旁路）+ 锁拆分。
  - 不同点：车规更看重最坏情况延迟（p99/p999/最大抖动）而非平均；要叠加 PREEMPT_RT（坦承没做过内核改）保证内核路径可抢占。
  - 测量方法迁移：perf/ftrace 测中断到用户态时间分布，看长尾而非平均——这套方法车规场景同样适用。
- 可追问"软中断绑核具体怎么做"：/proc/softirqs 看分布；smp_affinity(/proc/irq/NR/smp_affinity 位图)绑硬中断；RPS 设 /sys/class/net/ethX/queues/rx-N/rps_cpus 绑软中断；taskset 绑业务进程做 cache 亲和。

Q2：bpf 回合 1000 补丁怎么证明车规内核长期维护能力？怎么管回归？
答题要点（第二亮点）：
- 场景：4.19 内核要回合高版本 bpf/btf 特性，但同一基线要长期稳定演进（这正是车规内核的核心需求——ABI 稳定 + 特性持续回合）。
- 做了什么：回合特性 + 单测；解决校验器出错致近百用例失败；产出 btf 机制分享文档；合入 ~1000 补丁。
- 证明长期维护能力：
  - 补丁回合不是简单 cherry-pick，要处理上下文冲突、API 变更、依赖特性顺序、回归用例——这是"在同一基线上持续演进而不破坏稳定"的实战。
  - 回归管理：回合后跑 bpf 自测（tools/testing/selftests/bpf），近百用例失败要逐个定位是补丁问题还是测试框架问题；BTF 生成依赖 pahole 版本，CO-RE 相关宏变更要批量改用例。
  - 工程化：补丁系列 apply + build + test + report 可自动化，是车规内核 CI 的雏形。
- 迁移车规：车规内核要求一个基线维护 5-10 年，期间要持续回合安全补丁和必要特性而不破坏 ABI——我这 1000 补丁回合经验直接对应。
- 可追问"bpf 校验器为什么会报错"：4.19 校验器与高版本 bpf 字节码/CO-RE 机制不匹配，寄存器状态追踪/路径分析差异；回合时要同步校验器逻辑或限制新特性范围。
- 诚实边界：回合的是已有特性，不是从零设计 bpf 子系统；但"长期维护同一基线"的工程能力是实打实的。

Q3：kpatch 怎么对应车端 OTA 底层热修？产品化做了什么？
答题要点（第三亮点）：
- 车端场景：智驾域控上线后不能随便重启（影响可用性、要重新跑自检），关键内核 bug 要热修——这正是 kpatch/livepatch 的价值。
- 简历做了什么：
  - 调研红帽/欧拉/org 社区 kpatch 方案。
  - 仿红帽在欧拉开源基础上完成多版本 rpm spec 编写打包——把 patch 构建（kpatch-build）、签名、分发做成可分发产物。
  - 灰度回滚：先测试机 load → 灰度 N 台 → 全量；livepatch 支持 disable（echo > /sys/kernel/livepatch/<name>/disable）瞬间切回旧函数。
- kpatch 原理：基于 ftrace/livepatch，构造一致性模型——等旧函数所有线程退出栈后再切到新函数（grace period），stop_machine 保证原子性。
- 车端迁移点：
  - OTA 上注补丁 KB 级，比整机升级轻。
  - 灰度可对应"单车先加载验证→确认无异常→固化"。
  - 回滚对应"异常自动 disable 切回旧函数"。
- 局限：不能改数据结构布局（只改函数逻辑）、改不了 init 类已调用函数、依赖 CONFIG_LIVEPATCH。
- 诚实：做的是产品化（打包/分发/灰度流程），livepatch 内核机制本身是社区现成；车端 OTA 还涉及签名链/安全启动/版本管理，这部分坦承没做过但理解需要。

Q4：ARM RAS + bit 翻转怎么对应 ASIL 功能安全？ISO 26262 了解吗？
答题要点（差异化故事）：
- 诚实：ISO 26262 没做过项目，坦承未做过但理解原理；ASIL 分级（A/B/C/D，D 最严）和功能安全流程是理论了解。
- 简历经验：
  - 飞腾联合项目调研 ARM 架构 RAS：SDEI 中断机制及内核实现；实验验证内存错误注入（固件 PSCI/ERRIDR → SDEI 中断 → 内核 EDAC → rasdaemon 全链路）。
  - 现场排查：硬件随机单 bit 翻转 → do_page_fault，crash 看页表项被改对比正常。
- 对应 ASIL 的链路：
  - ASIL 要求硬件错误要"检测→上报→隔离/降级→可观测"，简历的 RAS 链路就是"检测（硬件 ECC/EDAC）→上报（SDEI/中断）→内核处理（EDAC/rasdaemon）→用户态可观测（sysfs）"。
  - 车规要做的是：检测后要触发降级（如隔离出错 CPU 核、降级运行模式、上报诊断码 DTC）——这部分坦承没做过，但理解是这套链路。
- 迁移点：征程 BPU + ARM 车规芯片同样面临硬件错误检测需求，我对 EDAC/SDEI/SEC-DED 上报链路有底层实验经验，比纯软件背景候选人更懂硬件容错。
- 可追问"SEC-DED 怎么纠错"：汉明码扩展一位校验位，单 bit 错纠正（知道哪一位翻）、双 bit 错检测但不能纠（上报为 UE）；ASIL-D 场景双 bit 错要触发安全状态（降级/复位）。

Q5：申威 io_uring 跨架构 Bug 怎么定位的？新征程 BPU 架构适应力？
答题要点（佐证跨架构适应力）：
- 案例：申威架构 io_uring fork io 线程段错误——架构代码 bug，io_uring 创建 io worker 线程时架构层（TLB/上下文切换/fork 路径）处理有问题。
- 定位手段：
  - crash vmcore：bt 看 io_uring worker 线程栈，定位到架构相关函数（如 switch_mm/fork 路径某处缺页或上下文未切对）。
  - 对比：同路径在 x86/ARM 上不触发，差异在架构层——这种 bug 要对照 arch/<arch>/ 下代码分析。
  - ftrace/eBPF 跨架构通用：trace fork/io_uring 路径，看哪一步异常。
- 跨架构方法论：
  - 工具链通用：crash/ftrace/perf/eBPF 在 ARM/x86/申威/BPU 上都可用，方法论可复用。
  - 架构差异点：内存管理（页表/TLB）、上下文切换、中断控制器、原子操作实现——这些是跨架构要重点对照的。
  - 新架构适应力：BPU/车规新架构内核同样会有架构层 bug，我能在不熟悉该架构时用通用方法论 + 对照 arch 代码定位。
- 其他跨架构案例：ARM 二层转发软中断占满 CPU（ebt_do_table 读写锁 + lse 原子锁）——ARM 上 LSE 原子指令实现差异导致的锁竞争，也是架构相关定位。
- 可追问"io_uring fork 为什么会段错误"：io_uring 的 io worker 线程是内核线程，fork 路径要正确继承/重置架构上下文（如 TLS/TLS 寄存器、地址空间标识）；架构层没正确处理会导致 worker 跑飞。
