...
> 本文件用于跨会话/跨 Agent 继承求职调研上下文。新会话的 AI（或候选人本人）读这一份 + `memory.md` 即可快速接续，不必重读全部 2158 行 companies.md。

最后更新：2026-07-16 17:00｜仓库：github.com/lucienwyb/find-job｜60 轮调研、投递目录 32 份材料 + 1 份执行总表

## ★当前状态（2026-07-16 17:00）
- 情报调研+投递材料阶段**彻底完成**：companies.md 2158行、9大赛道全覆盖、地址全部核实可信、投递目录32份"复制即发"材料 + 执行总表（`投递/投递-00-执行总表速查.md`）。
- **循环 job 进行中**：`43d4fd6a`（每20min 多agent并发），48h 后 2026-07-18 14:02 自动停（停止 job `93ddf8c5`）。
- **真正该动手投递了**——情报够用，照 `投递/投递-00-执行总表速查.md` 第一梯队顺序投即可。
...
### ★★ 投递目录（32份材料 + 1份执行总表，复制即发）
**照 `投递/投递-00-执行总表速查.md` 投最省事**——总表把32份按4梯队排序（第一梯队本周必投/第二梯队脉脉内推破学历/第三梯队邮件问岗/第四梯队躺平盯公告），含2周节奏+学历破局策略。
- 第一梯队（本周必投）：ISCAS双投(jiran@iscas.ac.cn) + 寒武纪高级系统软件(jobId 088a8453 mokahr直链) + 银河仿真(jobId 87b113dd 急) + 星动飞书 + 云杉hr@yunshan.net+GitHub PR
- 第二梯队（内推破学历）：月之暗面3岗(先脉脉内推锁3岗) + 寒武纪通信库 + NVIDIA + 安谋 + 银河5岗 + 字节Coze
- jobId直链可立即投：寒武纪高级系统软件088a8453 / 银河仿真87b113dd / 月之暗面存储4f904813(先内推)
- 其余材料覆盖：清程/深势/面壁/中科闻歌/一流/中科视拓/海天瑞声/光轮/思码逸/数据堂/倍赛/长亭/青藤/微步/龙芯/曙光/工行/中行/CETC/百度转岗/地平线2岗/星忆等

### 第54-60轮新增强命中岗（JD点名内核经验，优先投）
- ★寒武纪高级系统软件开发工程师(jobId 088a8453,mokahr直链在招) — JD逐条对口:Linux内核bootloader/init/FS/进程调度/内存管理/设备驱动+PCIE/USB/ETH+X86/Aarch64+SMMU/DMA/GIC+Bringup。本科+不卡211。
- ★地平线分布式存储研发工程师(7-15新发,hotjob需本人浏览器搜postId) — JD点名"内核/OS经验者优先""Linux I/O栈优先"。
- ★地平线智驾性能优化软件工程师(7-16新发,postId 6a587e89ba2dc64cbb6db192) — C++/Linux/嵌入式/ARM NEON。
- ★银河通用具身智能仿真平台(jobId 87b113dd,7-13急招) — Docker/K8s/Ray/Slurm。
...
### 核心文档（根目录，投递时用）
- `companies.md`（2158 行）— 全部公司调研总表 + 行动总表 + 投递执行清单
